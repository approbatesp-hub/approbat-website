...
const ModifierProduit = () => {
  ...
  let livraisonDefaut = {
    value: product.livraisonGratuite ? "oui" : "non",
    label: product.livraisonGratuite ? "oui" : "non",
  };

  const [livraisonGratuite, setLivraisonGratuite] = useState(livraisonDefaut);

  let enStockDefaut = {
    value: product.enStock ? "oui" : "non",
    label: product.enStock ? "oui" : "non",
  };
  ...
  let fournisseurDefaut = {
    value: product.fournisseur,
    label: product.fournisseur,
  };
  const [fournisseur, setFournisseur] = useState(fournisseurDefaut);
  ...
  let typesDefaut = product.types?.map((pro) => ({
    value: pro.type,
    label: pro.type,
  }));
  const [types, setTypes] = useState(typesDefaut);

  let prixDefaut = product.types?.map((pro) => pro.prix);
  const [prixTypes, setPrixTypes] = useState(prixDefaut);

  const [prixReference, setPrixReference] = useState(product.prixReference);
  ...
  const resizeFile = (file) =>
    new Promise((resolve) => {
      Resizer.imageFileResizer(
        ...
        0, // rotation
        (blob) => {
          // Wrap blob into a File to preserve name
  ...
  const uploadImages = async (e) => {
    ...
    for (const file of resizedFiles) {
      ...
      if (error) {
        ...
        // setImagesTab([...imagesTab, data.publicUrl]);
        setImagesTab((image) => [...image, data.publicUrl]);
      }
    }
  ...
  // IMAGE HANDLER END
  return (
    <div className="xl:max-w-[90%] mx-auto lg:p-6">
      <div className="flex gap-5 p-2 items-center mb-2">
      ...
      >
        <div className="grid grid-cols-1 lg:divide-x-2 lg:divide-dashed divide-gray-300  lg:grid-cols-[40%_58%] md:gap-8 p-4">
          ...
          <div className="basis-1/2 bg-white  h-full py-3 md:py-6">
            <div className="cursor-pointer p-6 md:p-0 h-[200px] md:h-[250px] lg:h-[40%] md:w-[90%] bg-slate-100 mx-auto rounded-md outline-2 outline-offset-2 outline-dashed outline-slate-300  flex  items-center justify-center relative ">
              {loading ? (
                <SimpleLoader />
              ) : (
                <div>
            ...
            <div className="mt-5 md:mt-0 md:p-5 space-y-3">
              {imagesTab.length > 0 &&
                imagesTab.map((item, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-1 shadow-sm bg-slate-100 p-2 rounded-md w)f"
                  >
                    <img
                      src={item}
                      alt=""
